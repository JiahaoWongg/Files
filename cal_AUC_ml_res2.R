...
#' )
#'
cal_AUC_ml_res2 <- function(res.by.ML.Dev.Prog.Sig = NULL, # ML.Dev.Prog.Sig, 函数计算结果
        ...
        if (num.b == num.a) {
          ...
          for (i in ml.names[c(which(ml.names %in% c("RSF")), grep("+ RSF", ml.names, fixed = T))]) {
            ...
            for(dataset in datasets){
              risk_score_tb = rs[[dataset]]
              if(length(unique(risk_score_tb$RS)) == 1){
                # cat(model, dataset, "\n")
                rs_real = risk_score_tb$RS
                rs_fix  = rs_real + runif(length(rs_real), min = 0.001, max = 0.01) * rs_real
                rs[[dataset]]$RS = rs_fix
              }
            }
